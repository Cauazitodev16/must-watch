...
class Entretenimento:
    """
        Classe para reoresentar uma tarefa, com métodos para salvar, obter, excluir e atualizar tarefas em um banco de dados usando a classe `Database`.
    """
    def __init__(self: Self, titulo_entretenimento: Optional[str] tipo_entretenimento: Optional[str] = None, id_entretenimento: Optional[int] = None) -> None:
        self.titulo_entretenimento: Optional[str] = titulo_entretenimento
        self.tipo_entretenimento: Optional[str] = tipo_entretenimento
        self.id_entretenimento: Optional[int] = id_entretenimento
    ...
    def id(cls, id: int) -> Self:
        with Database() as db:
            query: str = 'SELECT titulo_entretenimento, tipo_entretenimento FROM tarefas WHERE id = ?;'
            params: tuple = (id,)
